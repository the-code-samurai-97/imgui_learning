...
int main(int, char**) {
  ...
  //  io.Fonts->AddFontFromFileTTF("/home/sibi/.fonts/test.ttf", 16.0f);
  // ImFont* font =
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // to created a named window.
    {
      ...
      if (ImGui::Button("Add"))  // Buttons return true when clicked (most
                                 // widgets return true when edited/activated)
        counter++;
      ImGui::SameLine();
      if (ImGui::Button("Reset"))  // Buttons return true when clicked (most
                                   // widgets return true when edited/activated)
        counter = 0;
      if (ImGui::Button(
              "Negate"))  // Buttons return true when clicked (most widgets
